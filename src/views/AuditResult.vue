<template>
  <div class="auditResult">
    <div v-if="$store.state.driverRecruitData.auditState.state" class="userIsAuditing">
      <img :src="require('@/assets/icon_audit.png')">
      <span>我们会在一个工作日内进行初审，请耐心等待</span>
      <span>初审结果将以短信形式发送至您的手机，请注意查看</span>
    </div>
    <div v-if="!$store.state.driverRecruitData.auditState.state" class="userHasAudit">
      ...
      <img class="seal" v-if="$store.state.driverRecruitData.auditState.auditPass" :src="require('@/assets/icon_audit_pass.png')">
      <img class="seal" v-if="!$store.state.driverRecruitData.auditState.auditPass" :src="require('@/assets/icon_audit_reject.png')">
    </div>
  </div>
</template>
