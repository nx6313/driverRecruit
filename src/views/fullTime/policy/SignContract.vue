<template>
    ...
    <div class="signContract" v-if="userInfo">
        <div class="policyContentWrap">
            <p>本人 <u> {{userInfo.personName}} </u>，{{$comfun.formatDate(currentDate, 'yyyy')}} 年 {{$comfun.formatDate(currentDate, 'MM')}} 月 {{$comfun.formatDate(currentDate, 'dd')}} 日阅读并签阅了公司 《入职须知》、 《无犯罪记录声明》、《薪酬保密承诺书》、《驾驶人员交通安全承诺书》、《申报网约车驾驶员资格证承诺书》、《工衣西服费用申明》、《提交离职证明申明》、《无兼职工作申明》；我承诺将严格遵守上述文件中的各项规定，如有违反，愿意承担相应处罚以及相关责任。</p>
        </div>
