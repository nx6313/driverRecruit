<template>
    <div class="complete">
        ...
        <span v-if="$store.state.driverRecruit.state == $store.state.driverRecruit.stateRule.RULE_PASS" class="tip">您的大昌网约车自营司机申请已通过，正在等待入职<br>请您耐心等待或联系 <a :href="`tel:${contactPhone}`">{{contactPhone}}</a>（{{contactPeople}}） 咨询详情</span>
        <span v-if="$store.state.driverRecruit.state == $store.state.driverRecruit.stateRule.RULE_HAS_RUZHI" class="tip">您已成为大昌网约车自营司机，快去赚钱吧！</span>
        <span v-if="$store.state.driverRecruit.state == $store.state.driverRecruit.stateRule.RULE_NO_RUZHI" class="tip">您的大昌网约车自营司机申请已通过，但未入职<br>可联系 <a :href="`tel:${contactPhone}`">{{contactPhone}}</a>（{{contactPeople}}） 咨询详情</span>
